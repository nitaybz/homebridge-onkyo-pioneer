...
module.exports = {
	init: async function() {
		...
		for (const config of this.receivers) {
			...
			// Init onkyo.js
			const avr = new Onkyo({logger: this.log, address: config.ip, port: config.port, name: config.name})

			// Init Scan for device
			const onkyoScan = new OnkyoDiscover({logger: this.log, broadcastAddress: config.ip, broadcastPort: config.port})
			try {
				const detected = onkyoScan.discoverFirst()
			...
			if (deviceConfig) {
				...
				deviceConfig.main.volumeAccessory = config.volumeAccessory
				...
				deviceConfig.zone2.volumeAccessory = config.volumeAccessory
				deviceConfig.zone3.active = config.enableZone3
				deviceConfig.zone3.maxVolume = config.zone3MaxVolume
				deviceConfig.zone3.volumeAccessory = config.volumeAccessory
			} else {
				if (!avr.device.info) {
		...
		if (this.discover)
			startDiscovery(this.cachedDevices)
...
const startDiscovery = function(cachedDevices) {
	const onkyoScan = new OnkyoDiscover({logger: this.log})
	this.log('Starting discovery of supported Onkyo/Pioneer AVR...')
	onkyoScan.discover()

	onkyoScan.on('detected', (avr) => {
		if (!cachedDevices.find(cachedDevice => cachedDevice.address === avr.address)) {
