...
const newAVR = function(avr, deviceConfig, platform) {
	...
	avr.on('error', async (err) => {
		platform.log.error('ERROR OCCURRED:', err.message || err)
		try {
			platform.log.easyDebug('Trying to reconnect...')
			await avr._ensureConnection()
		} catch(err) {
			platform.log.error('Reconnect failed! Will try again in the next status update')
		}
